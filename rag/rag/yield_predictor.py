...
# Hardcoded encodings as provided by the user
ENCODINGS = {
...
def _get_model():
    ...
    # Try local directory (rag/), then root
        ...
        try:
            ...
            if isinstance(loaded, dict):
                if "model" in loaded:
                    ...
                    # If no 'model' key, check for other keys or use first value
                    print(f"[YieldPredictor] Dictionary keys: {list(loaded.keys())}")
                    if len(loaded) == 1:
                        _model_cache = list(loaded.values())[0]
                    else:
                        _model_cache = loaded # Still a dict, will likely fail but we've logged it
            else:
                _model_cache = loaded
